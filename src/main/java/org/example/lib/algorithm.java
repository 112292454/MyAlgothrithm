...
public class algorithm {
	...
	public static void main(String args[]) {
		out.println("ot.fastPower(3,19991018, (long) (1e9+7)) = " + ot.fastPower(3, 19991018L, (long) (1e9 + 7)));
...
class OtherTool {
	...
	//数字快速幂
	public static long fastPower(long source, long times, long mod) {
...
*/
	...
	private static int partition(int[] array, int l, int r) {
		int x = array[r];
		//x = array[l+new Random().nextInt(r-l-1)];
		int i = l - 1;
		for (int j = l; j < r; j++) {
			if (array[j] <= x) {
				i++;
	...
	public static int[] mergeSort(int[] array, int low, int high) {
		...
		//合并两个归并段
		return merge(array, low, high, mid);
	}
	...
	*/
	...
	public static int[] primes(int range) {
		...
		double nums = 0;
		if (range > 1000000) {
			nums = range * (Math.log(range) + 0.01);
		} else {
			nums = 80000;
		}
	...
	private static int[] KMPBuildNext(String s) {
		return KMPBuildNext(s, s);
	}
	...
	public static int getBinaryOneCount(int a) {
		return getBinaryOneCount((long) a);
	}
	...
	public static Node[] convexHull(Node[] arr) {
		...
		Arrays.sort(arr, (o1, o2) -> {
			double d = new Vector(o.x, o.y, o1.x, o1.y).CrossProduct(new Vector(o.x, o.y, o2.x, o2.y));
			if (d > 0) {
				return -1;
			} else if (d < 0) {
				return 1;
			} else {
				return distance(o.x, o.y, o1.x, o1.y) < distance(o.x, o.y, o2.x, o2.y) ? -1 : 1;
			}
		});
		res[0] = arr[0];
		...
		for (int i = 3; i < arr.length; i++) {
			while (new Vector(res[top - 1].x, res[top - 1].y, res[top].x, res[top].y).CrossProduct(new Vector(res[top].x, res[top].y, arr[i].x, arr[i].y)) < 0 && top > 1) {
				top--;
			}
			res[++top] = arr[i];
		}
...
class BipartiteGraph  extends   AdjacencyTable{
    ...
    public boolean canMatch(int from){
        ...
        int target=n.nowConnectNode;
        while (!used[target]&&n.nextConnectNode!=null) {n=n.nextConnectNode;target=n.nowConnectNode;}
        if(used[target]){
            used[target]=false;
            if(matched[target]==-1||canMatch(matched[target])){
                matched[target]=from;
                return true;
            }
        }
...
//			return distance[end];
//		} else {
//			return Integer.MAX_VALUE;//若返回maxint，则为不连通
//		}
//	}
//
//
////	/**
////	 * 二维数组表示图的邻接矩阵，f[i][j] 存储节点 i 到节点 j 的距离。
////	 */
////	double[][] f;
////
////	/**
...
////	 * @param start 起始节点的索引
////	 * @param end 目标节点的索引
////	 * @return 最短路径的长度，若两节点不连通返回 Integer.MAX_VALUE
////	 */
////	public int dijkstra(int start, int end) {
////		// 距离数组，存储起始节点到各节点的最短距离
////		int[] distance = new int[node.length];
////
////		// 记录节点是否已被访问
////		int[] used = new int[node.length];
////
////		// 优先队列，用于选择当前距离最短的节点
////		PriorityQueue<Node> nodeQueue = new PriorityQueue<>(Comparator.comparingInt(o -> o.value));
////
////		// 初始化距离数组，将所有距离设置为无穷大
////		Arrays.fill(distance, Integer.MAX_VALUE / 2);
////
////		// 将起始节点加入优先队列，起始节点到自身的距离为0
...
////			// 获取当前距离最短的节点
////			int currentCity = nodeQueue.poll().target;
////
////			// 若节点已被访问，则跳过
////			if (used[currentCity] != 0) continue;
////
////			// 标记当前节点为已访问
////			used[currentCity] = 1;
////
////			// 获取当前节点相邻的节点列表
////			ArrayList<Node> adjacentNodes = map[currentCity];
////
////			// 若当前节点没有相邻节点且不是目标节点，则图不连通，返回-1
////			if (adjacentNodes.isEmpty() && currentCity != end) return -1;
////
////			// 遍历当前节点的相邻节点
////			for (Node neighbor : adjacentNodes) {
////				int neighborCity = neighbor.target;
////
////				// 如果通过当前节点到达相邻节点的距离比已记录的距离短，则更新距离数组并加入优先队列
////				if (distance[neighborCity] > distance[currentCity] + neighbor.value) {
////					distance[neighborCity] = distance[currentCity] + neighbor.value;
////					nodeQueue.offer(new Node(neighborCity, distance[neighborCity]));
////				}
////			}
////		}
////
////		// 若目标节点的距离不是无穷大，则返回最短路径的长度，否则返回 Integer.MAX_VALUE 表示不连通
////		if (distance[end] != Integer.MAX_VALUE / 2) {
////			return distance[end];
////		} else {
...
class ShortestRoad {
	...
	public ShortestRoad(double[][] map) {
		this.map = map;
	}

	public int dijkstra(int start, int end) {
		int[] distance = new int[map.length];//start到各个点的最短距离
		int[] used = new int[map.length];
		PriorityQueue<Node> nodeQueue = new PriorityQueue<>(Comparator.comparingInt(o -> o.value));//待操作节点，利用他们去松弛distance
		...
		while (!nodeQueue.isEmpty()) {
			int city = nodeQueue.poll().to;
			if (used[city] != 0) continue;
			used[city] = 1;
			double[] neighbors = map[city];
			if (isEmpty(neighbors) && city != end) return Integer.MAX_VALUE;
			for (int i = 0; i < neighbors.length; i++) {
				if (neighbors[i] > 0 && distance[i] > distance[city] + neighbors[i]) {
					distance[i] = distance[city] + (int) neighbors[i];
					nodeQueue.offer(new Node(i, distance[i]));
				}
			}
		}
		...
		for (int k = 0; k < max; k++) {
			for (int i = 0; i < max; i++) {
				if (map[i][k] != Integer.MAX_VALUE / 2) {
					for (int j = 0; j < max; j++) {
						if (map[k][j] != Integer.MAX_VALUE / 2) {
...
class TreeToLink extends AdjacencyTable {
	...
	private void dfs1(int nowId, int fId, int depth) {
		...
		for (Node road : map[nowId]) {
			if (road.target == fId) continue;
			dfs1(road.target, nowId, depth + 1);
			s[nowId].treeSize += s[road.target].treeSize;
			if (s[road.target].treeSize > s[s[nowId].son].treeSize) s[nowId].son = road.target;
		}
	}
	...
	private void dfs2(int nowId, int top) {
		...
		for (Node road : map[nowId]) {
			if (road.target != s[nowId].son && road.target != s[nowId].father) dfs2(road.target, road.target);
		}
	}
...
class StronglyConnectComponent extends AdjacencyTable {
	...
	public void dfsStack(int nowNode) {
		...
		for (Node n : reMap[nowNode]) {
			if (!used1[n.target]) dfsStack(n.target);
		}
	...
	public void dfsResult(int nowNode, int index) {
		...
		for (Node n : map[nowNode]) {
			if (!used2[n.target]) dfsResult(n.target, index);
		}
...
class Fraction {
	...
	public Fraction add(Fraction second) {
		return new Fraction(this.Numerator * second.Denominator + second.Numerator * this.Denominator, this.Denominator * second.Denominator);
	}
